...
/*   Updated: 2023/08/31 03:27:19 by anaji            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_data(t_mlx **mlx, t_list *lines)
{
	t_list	*tmp;
	char	*buff;
	char	**array;
	...
	while (tmp)
	{
		...
		if (ft_strlen(array[0]) == 2)
			ft_lstadd_back(&(*mlx)->texture, ft_lstnew(create_texture(array)));
		if (!ft_strncmp(array[0], "F", 0))
			(*mlx)->color.f = convert_rgb(array[1]);
		else if (!ft_strncmp(array[0], "C", 0))
			(*mlx)->color.c = convert_rgb(array[1]);
		ft_free_array(array, 0);
		tmp = tmp->next;
	}
	return ;
